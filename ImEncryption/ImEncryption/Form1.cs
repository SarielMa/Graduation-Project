...
namespace ImEncryption
{
    public partial class Form1 : Form
    {
        ...
        private void 像素改变率NPCRToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (curBitmap != null)
            {
                int[,] Ph1 = comm.getPixel(0, pictureBox1, N);
                MessageBox.Show("本测试结果由两次加密所生成，为保证结果正确，请之后输入两次相同的分解层数，谢谢！");
                TEST test = new TEST();
                    
                    int[][,] C = encForDiff(Ph1, N);
        ...
        private void hill加密ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (curBitmap != null)
            {
                if (iw == N && ih == N)
                {
                    this.Text = "自可逆矩阵Hill加密系统";
                    P = comm.getPixel(0, pictureBox1, N);           //从窗口1读入图像 
                    int[,] E = (new HillEncryption()).HillEnc(P, N);//加密
                   
                    comm.showImage("加密图", E, E, E, 87, 60, pictureBox1, label1, N);
                }
                else
                    MessageBox.Show("仅仅适用于256X256图像!", "信息提示");
            }
        ...
        private void hill解密ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (curBitmap != null)
            {
                int[,] E2 = comm.getPixel(0, pictureBox1, N);     //从窗口1读入图像 
                 int[,] Po = (new HillEncryption()).HillDec(E2, N);//解密
                comm.showImage("解密图", Po, Po, Po, 387, 60, pictureBox2, label2, N);
                comm.imgDiff("原图", "解密图", P, Po, 256);      //比较原图P与重构图dP像素差异
                recover();
            }
