...
namespace ImEncryption
{
    class HaarWT
    {
        ...
        public int[,] lwavedec2(int[,] image, int N)//一次2d小波提升分解
        {
            int T = N / 2;
            int[,] f = new int[N, T];
            int[,] f2 = new int[N, T];
            ...
            for (int k = 0; k < T; k++)
            {
                for (int l = 0; l < N; l++)
                {
                    hcol[l, k] = f[l, k] - f2[l, k];//得出的是D，对奇数提升
                }
            }
            for (int m = 0; m < T; m++)
            {
                for (int n = 0; n < N; n++)
                {
                    lcol[n, m] = f2[n, m] + (int)Math.Floor(0.5 * (double)hcol[n, m] + 0.5);//的出来的是S，主要提升，所选U为1/2d+1/2
                }
            }
